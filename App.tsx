...
import { generateLayout, generateText, generateImage } from './services/geminiService';
...
const INITIAL_ELEMENTS: PageElement[] = TEMPLATES[0].elements;

export default function App() {
  const [elements, setElements] = useState<PageElement[]>(INITIAL_ELEMENTS);
  ...
  // Persist templates
  useEffect(() => {
      localStorage.setItem('buildflow_templates', JSON.stringify(savedTemplates));
  }, [savedTemplates]);
      ...
      if (editingTemplateId) {
          setSavedTemplates(prev => prev.map(t => {
              if (t.id === editingTemplateId) {
                   ...
                   // Template root must remain single, but our logic handles arrays. 
                   // We assume the root is index 0.
  ...
  const handleEditTemplate = (templateId: string) => {
      ...
      // Select the root of the template
  ...
  const handleDeleteTemplate = (id: string) => {
      // Direct delete, no confirmation as requested
      setSavedTemplates(prev => prev.filter(t => t.id !== id));
      showToast("Template deleted");
  };
  
  const handleDetachElement = (id: string) => {
      // Convert Global Instance -> Local Deep Copy
      ...
      // Replace the global placeholder with the deep copy content
      setActiveElements(prev => {
           const replaceRecursively = (list: PageElement[]): PageElement[] => {
               return list.map(item => {
                   ...
                   if (item.children) {
                       return { ...item, children: replaceRecursively(item.children) };
                   }
                   return item;
               });
           };
  ...
  const handleDropElement = (targetId: string, position: 'inside' | 'after' | 'before', data: any) => {
      ...
      if (data.type === 'new') {
          ...
          if (template.isGlobal) {
              ...
              newElement.name = template.name; // Keep name
          }
  ...
  const handleGenerateLayout = async () => {
    if (!aiPrompt) return;
    setIsAiLoading(true);
    const newElements = await generateLayout(aiPrompt);
    if (newElements.length > 0) {
      ...
      showToast("Layout generated successfully!");
        ...
        showToast("Failed to generate layout. Try again.");
    }
    setIsAiLoading(false);
  };

  const handleAiTextImprovement = async () => {
    if (!selectedId) return;
    const activeList = getActiveElements();
    const el = findElement(selectedId, activeList);
    if (!el || (el.type !== 'text' && el.type !== 'button' && el.type !== 'heading')) return;

    setIsAiLoading(true);
    const newText = await generateText("Improve this text to be more engaging and professional.", el.props.content);
    handleUpdateProps(selectedId, { content: newText });
    setIsAiLoading(false);
  ...
  const handleAiImageGeneration = async () => {
    ...
    setPromptConfig({
        ...
        defaultValue: "A futuristic city skyline",
        onConfirm: async (prompt) => {
            setIsAiLoading(true);
            const base64Image = await generateImage(prompt);
            if (base64Image) {
                handleUpdateProps(selectedId, { src: base64Image });
  ...
  const handleAiBackgroundGen = async (propName: 'backgroundImage' | 'backgroundVideo') => {
      if (!selectedId) return;
      
      setPromptConfig({
        isOpen: true,
        title: "Generate Background",
        defaultValue: "Abstract gradient pattern",
        onConfirm: async (prompt) => {
            setIsAiLoading(true);
            const base64Image = await generateImage(prompt); 
            if (base64Image) {
                handleUpdateStyle(selectedId, propName, base64Image);
                showToast("Background updated!");
            } else {
                showToast("Failed to generate background.");
            }
            setIsAiLoading(false);
        }
    });
  ...
  const handleSave = () => {
      const flatten = (items: PageElement[]): any[] => {
          let flatList: any[] = [];
          items.forEach(item => {
      ...
      showToast("Layout saved! Check console for JSON.");
  };
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {!isPreview && (
          <PropertiesPanel
            ...
            onAiText={handleAiTextImprovement}
            onAiImage={handleAiImageGeneration}
            onAiBg={handleAiBackgroundGen}
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden animate-fade-in">
            <div className="p-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                    <h3 className="text-lg font-bold text-gray-900">Generate Section</h3>
                    <p className="text-sm text-gray-500">Describe what you want to build.</p>
                </div>
              </div>
              <textarea
                className="w-full border border-gray-200 rounded-lg p-3 text-sm focus:ring-2 focus:ring-violet-500 focus:outline-none min-h-[120px] resize-none"
                placeholder="e.g. A pricing table with 3 cards, blue background, and rounded corners..."
                value={aiPrompt}
              ...
              <div className="mt-6 flex justify-end gap-3">
                ...
                <button 
                  ...
                  className="px-6 py-2 bg-violet-600 hover:bg-violet-700 text-white rounded-lg text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                >
                  {isAiLoading ? 'Generating...' : 'Generate Layout'}
                </button>
              </div>
            </div>
          </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {TEMPLATES.map(t => (
                            <button 
                                key={t.name}
                                onClick={() => {
                                    // Direct load, no confirm
                                    setElements(t.elements);
                                    setIsTemplatesModalOpen(false);
                                    showToast(`${t.name} loaded successfully`);
                                }}
