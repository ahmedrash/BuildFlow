...
<body class="bg-white text-slate-900 overflow-x-hidden">
    ...
    <script type="text/babel">
        ...
        const Icons = {
             ...
             ChevronUp: (props) => <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}><path d="m18 15-6-6-6 6"/></svg>,
        ...
        const ElementRenderer = ({ element }) => {
            ...
            switch (element.type) {
                case 'menu':
                    ...
                    const menuBreakpoints = { 'sm': 'sm', 'md': 'md', 'lg': 'lg', 'none': 'none' };
                    const bp = menuBreakpoints[mobileMenuBreakpoint] || 'md';
                    const bpClass = bp === 'none' ? 'flex' : \`hidden \${bp}:flex\`;
                    const mobileClass = bp === 'none' ? 'hidden' : \`flex \${bp}:hidden\`;
                         ...
                         <div className={\`flex items-center \${innerClass}\`} style={{...innerStyle}}>
                             ...
                             {isMenuOpen && (
                                ...
                                {mobileMenuType === 'dropdown' && (
                                    <div className={\`absolute top-full left-0 w-full bg-white shadow-lg border-t border-gray-100 flex flex-col z-40 max-h-[80vh] overflow-y-auto \${isClosing ? 'animate-fade-out' : 'animate-fade-in'} \${bp === 'none' ? 'hidden' : \`\${bp}:hidden\`}\`} style={{ backgroundColor: menuBackgroundColor || 'white' }}>
                                ...
                                {(mobileMenuType === 'slide-left' || mobileMenuType === 'slide-right') && (
                                    <div className={\`fixed inset-0 z-50 h-[100vh] \${bp === 'none' ? 'hidden' : \`\${bp}:hidden\`}\`}>
                ...
                case 'button':
                    ...
                    const { buttonIconLeft, buttonIconRight, buttonIsIconOnly } = element.props;
                    const renderButtonContent = () => {
                         if (buttonIsIconOnly) return <Icons.Box />;
                         return <div className="flex items-center justify-center gap-2">
                            {buttonIconLeft && <Icons.Box width={16} height={16} />}
                            <span>{element.props.content || 'Button'}</span>
                            {buttonIconRight && <Icons.Box width={16} height={16} />}
                         </div>;
                    };
                    if (action === 'link') {
                        return <a href={element.props.href} target={element.props.target} className={\`px-4 py-2 rounded transition inline-block text-center \${customClass} \${innerClass}\`} style={{...element.props.style, ...innerStyle}}>{renderButtonContent()}</a>;
                    }
                    if (action === 'popup') {
                         return <button type="button" className={\`px-4 py-2 rounded transition cursor-pointer \${customClass} \${innerClass}\`} style={{...element.props.style, ...innerStyle}} onClick={(e) => { e.preventDefault(); if(element.props.popupTargetId) openPopup(element.props.popupTargetId); }}>{renderButtonContent()}</button>;
                    }
                    return <button type="submit" className={\`px-4 py-2 rounded transition \${customClass} \${innerClass}\`} style={{...element.props.style, ...innerStyle}}>{renderButtonContent()}</button>;
                case 'logo':
                    ...
                    return <a href={element.props.href} className="block">{logoContent}</a>;
                case 'input': 
                ...
                case 'list':
                    ...
                    const listItems = element.props.listItems || (element.props.items || []).map((t, i) => ({ id: i, text: t }));
                    return (<ListTag className={\`pl-5 \${innerClass}\`} style={{ listStyleType: element.props.listStyleType || 'disc', ...innerStyle }}>{listItems.map((item, i) => <li key={item.id || i} style={{ marginBottom: element.props.itemSpacing }}>{item.text}</li>)}</ListTag>);
                case 'map':
                ...
                case 'gallery':
                     ...
                     if (galleryLayout === 'flex') return (<div className={\`flex flex-wrap \${innerClass}\`} style={gapStyle}>{galleryImages.map(img => (<div key={img.id} className={\`flex-grow basis-64 min-w-[200px] \${galleryAspectRatio === 'auto' ? '' : galleryAspectRatio} relative\`}><img src={img.src} alt={img.alt || ''} className={\`w-full h-full rounded absolute inset-0 object-\${galleryObjectFit}\`} /></div>))}</div>);
                     return (<div className={\`grid \${gridCols[galleryColumnCount]} \${innerClass}\`} style={gapStyle}>{galleryImages.map(img => (<div key={img.id} className={\`relative overflow-hidden rounded \${galleryAspectRatio}\`}><img src={img.src} alt={img.alt || ''} className={\`w-full h-full rounded object-\${galleryObjectFit}\`} /></div>))}</div>);
                case 'testimonial':
                ...
                case 'customScript':
                    ...
                    React.useEffect(() => {
                        if (element.props.scriptContent && scriptRef.current) {
                            try {
                                const func = new Function('gsap', 'ScrollTrigger', 'document', 'window', element.props.scriptContent);
                                func(gsap, ScrollTrigger, document, window);
                                ScrollTrigger.refresh();
                            } catch (e) { console.error('Script Error:', e); }
                        }
                    }, [element.props.scriptContent]);
                    return <div ref={scriptRef} style={{ display: 'none' }} />;
                default:
                    return null;
            }
        };
        ...
        const SliderRenderer = ({ element }) => {
            ...
            React.useEffect(() => {
                if (element.props.sliderAutoplay && element.children?.length > 1) {
            ...
            return (
                <React.Fragment>
                    {element.children?.map((child, index) => {
                         ...
                         let effectClass = isActive ? 'opacity-100' : 'opacity-0';
                         if (transition === 'zoom') effectClass = isActive ? 'opacity-100 scale-100' : 'opacity-0 scale-110';
                         if (transition === 'slide-up') effectClass = isActive ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8';
                         return (
                            <div key={child.id} className={\`w-full h-full transition-all duration-700 ease-in-out \${isActive ? 'relative z-10' : 'absolute top-0 left-0 z-0'} \${effectClass} \${isActive ? '' : 'pointer-events-none'}\`}>
        ...
        const PageElementRenderer = ({ element }) => {
            ...
            React.useEffect(() => {
                ...
                const handleScroll = () => {
                    ...
                    if (window.scrollY > offset) { if (stateRef.current !== 'stuck') setStickyState('stuck'); }
                    else { if (stateRef.current === 'stuck') { setStickyState('unsticking'); setTimeout(() => setStickyState(p => p === 'unsticking' ? 'idle' : p), 300); } }
                };
            ...
            let stickyClass = '';
            if (type === 'navbar') {
                stickyClass = headerType === 'fixed' ? 'fixed top-0 left-0 w-full z-50' : (headerType === 'sticky' ? (stickyState === 'stuck' ? 'fixed top-0 left-0 w-full z-50 animate-slide-in-down shadow-md' : (stickyState === 'unsticking' ? 'fixed top-0 left-0 w-full z-50 animate-slide-out-up shadow-md' : 'relative')) : 'relative');
            }
        ...
        const PageRenderer = ({ elements, savedTemplates }) => {
            ...
            return (
                <PopupContext.Provider value={{ openPopup, popupTargets, megaMenuTargets }}>
                         ...
                         {activePopupId && activePopupElement && ReactDOM.createPortal(
                             <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
                                 ...
                                 <div className="relative shadow-2xl overflow-hidden w-auto max-w-full max-h-[90vh] overflow-y-auto">
                                      ...
                                      {/* Render Popup content without trigger hiding */}
                                      <PopupContext.Provider value={{ openPopup, popupTargets: new Set(), megaMenuTargets: new Set() }}>
