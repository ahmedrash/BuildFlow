    ...
    <script type="text/babel">
        ...
        const ElementRenderer = ({ element }) => {
            ...
            switch (element.type) {
                ...
                case 'button': {
                    ...
                    if (action === 'popup') {
                        return <button onClick={() => openPopup(element.props.popupTargetId)} className={"px-4 py-2 rounded transition " + (element.props.className || '')} style={element.props.style}>{element.props.content}</button>;
                    }
                    if (action === 'link') return <a href={element.props.href} target={element.props.target} className={"px-4 py-2 rounded transition inline-block " + (element.props.className || '')} style={element.props.style}>{element.props.content}</a>;
                    return <button className={"px-4 py-2 rounded transition " + (element.props.className || '')} style={element.props.style}>{element.props.content}</button>;
                }
                case 'list': {
        ...
        const PageRenderer = ({ elements, savedTemplates }) => {
            ...
            const renderElement = (element, isPopupContent = false) => {
                ...
                const LinkWrapper = ({children}) => { if (type === 'card' && props.cardLink) return <a href={props.cardLink} className="block h-full no-underline text-inherit">{children}</a>; return <>{children}</>; };
                ...
                return (
                    <Tag key={id} className={containerClasses + ' ' + classNameToApply + extraClasses} style={{...props.style, ...bgStyle}} {...formProps}>
                         ...
                         <LinkWrapper>
                            ...
                            {children && children.length > 0 ? children.map(child => renderElement(child, isPopupContent)) : <ElementRenderer element={renderedElement} />}
                         </LinkWrapper>
                    </Tag>
                );
            };

            const popupElement = activePopupId ? findElement(activePopupId, elements) : null;

            return (
                <PopupContext.Provider value={{ openPopup, popupTargets }}>
                    {elements.map(el => renderElement(el))}
                    {activePopupId && popupElement && ReactDOM.createPortal(
                        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 animate-fade-in">
                            ...
                            <div className="relative shadow-2xl overflow-hidden w-full max-h-[90vh] overflow-y-auto animate-fade-in-up">
                                ...
                                {renderElement(popupElement, true)}
                            </div>
                        </div>,
                        document.body
                    )}
