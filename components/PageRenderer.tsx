...
const RenderedPageElement: React.FC<{
  ...
  megaMenuTargets: Set<string>;
    ...
    const NavbarWrapper: React.FC<{children: React.ReactNode}> = ({ children }) => {
        ...
        return <div ref={elementRef as any} id={id} className={`${props.className || ''} ${stickyClass}`} style={props.style}>{children}</div>;
    };
    ...
    if (type === 'slider' && children) {
        return (
            <Tag ref={elementRef} key={id} className={`${props.className || ''} ${overflowClass}${hiddenClass}`} style={props.style}>
                {renderBackground()}
                <SliderRenderer element={renderedElement} savedTemplates={savedTemplates} isPreview={isPreview} popupTargets={popupTargets} megaMenuTargets={megaMenuTargets} />
            </Tag>
        );
    }
    ...
    if (type === 'navbar') {
        return (
            <NavbarWrapper key={id}>
                {renderBackground()}
                <LinkWrapper>
                    {children && children.length > 0 ? children.map(child => <RenderedPageElement key={child.id} element={child} savedTemplates={savedTemplates} isPreview={isPreview} popupTargets={popupTargets} megaMenuTargets={megaMenuTargets} />) : <ElementRenderer element={renderedElement} isPreview={isPreview} />}
                </LinkWrapper>
            </NavbarWrapper>
        )
    }
    ...
    return (
        <Tag ref={elementRef} key={id} id={id} className={`${props.className || ''} ${overflowClass} ${getCardHoverClass()}${hiddenClass}`} style={props.style} {...formProps}>
...
export const PageRenderer: React.FC<PageRendererProps> = ({ elements, savedTemplates = [], isPreview = true }) => {
  ...
  return (
    <PopupContext.Provider value={{ openPopup, popupTargets }}>
        <PageContext.Provider value={{ findElement: (id) => findElementById(id, elements) }}>
            {elements.map(el => <RenderedPageElement key={el.id} element={el} savedTemplates={savedTemplates} isPreview={isPreview} popupTargets={popupTargets} megaMenuTargets={megaMenuTargets} />)}
                ...
                <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 animate-fade-in">
                    ...
                    <div className="relative shadow-2xl overflow-hidden w-auto max-w-full max-h-[90vh] overflow-y-auto animate-fade-in-up">
                        ...
                        <div className="popup-content-wrapper">
                            <div className="!block [&_.hidden]:!block"><div style={{ display: 'block !important' }}>
                                <RenderedPageElement element={activePopupElement} savedTemplates={savedTemplates} isPreview={isPreview} popupTargets={popupTargets} megaMenuTargets={megaMenuTargets} />
                            </div></div>
                        </div>
                    </div>
                </div>,
...
const SliderRenderer: React.FC<{ element: PageElement; savedTemplates: SavedTemplate[]; isPreview: boolean; popupTargets: Set<string>; megaMenuTargets: Set<string>; }> = ({ element, savedTemplates, isPreview, popupTargets, megaMenuTargets }) => {
            ...
            {element.children.map((child, index) => {
                ...
                return (
                    <div key={child.id} className={`${commonClass} ${posClass} ${effectClass} ${isActive ? '' : 'pointer-events-none'}`}>
                        <RenderedPageElement key={child.id} element={child} savedTemplates={savedTemplates} isPreview={isPreview} popupTargets={popupTargets} megaMenuTargets={megaMenuTargets} />
                    </div>
                )
            })}
                ...
                <>
                    ...
                    {element.props.sliderShowPagination !== false && (
                        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-20 flex gap-2">
                            {element.children.map((_, i) => <button key={i} className={`w-2 h-2 rounded-full transition-all ${i === activeIndex ? 'bg-white scale-125' : 'bg-white/50'}`} onClick={() => setActiveIndex(i)} />)}
                        </div>
                    )}
                </>
            )}
